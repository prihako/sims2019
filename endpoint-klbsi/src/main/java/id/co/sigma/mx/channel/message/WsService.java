...
public class WsService implements Transport<WsMessage>,InitializingBean,DisposableBean {
	...
	private String httpUrlKLBSIInquiry;

	public void setHttpUrlKLBSIInquiry(String httpUrlKLBSIInquiry) {
		this.httpUrlKLBSIInquiry = httpUrlKLBSIInquiry;
	}

	public void setKlbsiService(PaymentManagerControllerPortType paymentManagerService) {
	...
	public void write(WsMessage message) {
		...
		if(message.getValue("transactionType").equalsIgnoreCase("KLBSI.INQ")){
			...
			LOGGER.trace("+++Receive response from  KLBSI : " + response);
		...
		}else if(message.getValue("transactionType").equalsIgnoreCase("KLBSI.PAY")){
			...
			LOGGER.trace("+++Sent message payment to KLBSI+++");
			...
			LOGGER.trace("+++Receive response payment from KLBSI : " + response);

			message.setValue("errorCode", response.getStatus().getErrorCode());
			message.setValue("responseCode", response.getStatus().getErrorCode());
		}else if(message.getValue("transactionType").equalsIgnoreCase("KLBSI.REV")){
			StringArray stringArray = new StringArray();
			ArrayOfString arrayOfString = new ArrayOfString();
			stringArray.getItem().add(message.getValue("paidBills"));
			arrayOfString.setStrings(stringArray);
			...
			LOGGER.trace("+++Sent request Reversal to  KLBSI+++");
			...
			LOGGER.trace("+++Receive response reversal from  KLBSI : " + response);
	...
	private void tellListeners(WsMessage response) {
		for (MessageListener<WsMessage> listener : listeners) {
			if(LOGGER.isTraceEnabled())
				LOGGER.trace("Notify MX for response message from KLBSI");
	...
	public void destroy() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Shutting Down KLBSI Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "disconnected");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("KLBSI Endpoint successfully stopped");
	}
	...
	public void afterPropertiesSet() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Starting Up KLBSI Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "ready");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("KLBSI Endpoint successfully started");
	}
...
//		PaymentManagerControllerService client = (PaymentManagerControllerService) appContext.getBean("klbsi");
