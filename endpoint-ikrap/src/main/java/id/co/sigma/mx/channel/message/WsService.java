...
import id.co.sigma.mx.channel.endpoint.ArrayOfString;
import id.co.sigma.mx.channel.endpoint.InquiryRequest;
import id.co.sigma.mx.channel.endpoint.InquiryResult;
import id.co.sigma.mx.channel.endpoint.PaymentManagerControllerPortType;
import id.co.sigma.mx.channel.endpoint.PaymentRequest;
import id.co.sigma.mx.channel.endpoint.PaymentResult;
import id.co.sigma.mx.channel.endpoint.ReversalRequest;
import id.co.sigma.mx.channel.endpoint.ReversalResult;
import id.co.sigma.mx.channel.endpoint.StringArray;
...
public class WsService implements Transport<WsMessage>,InitializingBean,DisposableBean {
	...
	public void write(WsMessage message) {
		...
		if (LOGGER.isTraceEnabled()) {
			LOGGER.trace("###Receiving message from MX At IKRAP Endpoint###");
		}
		
		if(message.getValue("transactionType").equalsIgnoreCase("IKRAP.INQ")){
			...
			LOGGER.trace("+++Receive response from  IKRAP : " + response.toString());
		...
		}else if(message.getValue("transactionType").equalsIgnoreCase("IKRAP.PAY")){
			...
			paymentRequest.setBillKey3(message.getValue("billKey3"));
			...
			LOGGER.trace("+++Sent message payment to IKRAP+++");
			...
			LOGGER.trace("+++Receive response payment from  IKRAP : " + response);

			message.setValue("errorCode", response.getStatus().getErrorCode());
			message.setValue("responseCode", response.getStatus().getErrorCode());
		}else if(message.getValue("transactionType").equalsIgnoreCase("IKRAP.REV")){
			StringArray stringArray = new StringArray();
			ArrayOfString arrayOfString = new ArrayOfString();
			stringArray.getItem().add(message.getValue("paidBills"));
			arrayOfString.setStrings(stringArray);
			...
			LOGGER.trace("+++Sent request Reversal to  IKRAP+++");
			...
			LOGGER.trace("+++Receive response reversal from  IKRAP : " + response);
	...
	private void tellListeners(WsMessage response) {
		for (MessageListener<WsMessage> listener : listeners) {
			if(LOGGER.isTraceEnabled())
				LOGGER.trace("Notify MX for response message from IKRAP");
	...
	public void destroy() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Shutting Down IKRAP Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "disconnected");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("IKRAP Endpoint successfully stopped");
	}
...
//	};
	...
	public void afterPropertiesSet() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Starting Up IKRAP Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "ready");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("IKRAP Endpoint successfully started");
