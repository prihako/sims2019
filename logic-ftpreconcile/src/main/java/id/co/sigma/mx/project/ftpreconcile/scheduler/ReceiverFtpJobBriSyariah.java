...
import id.co.sigma.mx.project.ftpreconcile.process.ExportToXlsBriSyariah;
import id.co.sigma.mx.project.ftpreconcile.process.ReceiverFTPBriSyariah;

public class ReceiverFtpJobBriSyariah extends QuartzJobBean{

	protected static transient Logger logger = Logger.getLogger(ReceiverFtpJobBriSyariah.class);

	private ReceiverFTPBriSyariah receiverFTP;
	private ExportToXlsBriSyariah exportToXls;
	private DataSource dataSource;
	private static boolean isRunning = false;

	public void setReceiverFTP(ReceiverFTPBriSyariah receiverFTP) {
		this.receiverFTP = receiverFTP;
	}
	
	public void setExportToXls(ExportToXlsBriSyariah exportToXls) {
		...
		while (isRunning) {
			try {
				...
				for(JobMT940 job : listJob){
					...
					receiverFTP.execute(job.getTypeTrx(), job.getIsSameAccountNo(), job.getFilePattern(), job.getTransactionCode(), job.getEndpointCode(), "bri_syariah");
					exportToXls.execute(job.getAccountNo(), job.getTypeTrx(), job.getTransactionCode(), job.getIsSameAccountNo(), "bri_syariah");
				}
	...
	public List<JobMT940> getJobs(){
		...
		try {
			...
			String sql = "SELECT * from job_mt940_bri_sya";
