...
import id.co.sigma.mx.project.ftpreconcile.process.ExportToXlsBniSyariah;
import id.co.sigma.mx.project.ftpreconcile.process.ReceiverFTPBniSyariah;

public class ReceiverFtpJobBniSyariah extends QuartzJobBean{

	protected static transient Logger logger = Logger.getLogger(ReceiverFtpJobBniSyariah.class);

	private ReceiverFTPBniSyariah receiverFTP;
	private ExportToXlsBniSyariah exportToXls;
	private DataSource dataSource;
	private static boolean isRunning = false;

	public void setReceiverFTP(ReceiverFTPBniSyariah receiverFTP) {
		this.receiverFTP = receiverFTP;
	}
	
	public void setExportToXls(ExportToXlsBniSyariah exportToXls) {
		...
		while (isRunning) {
			try {
				...
				for(JobMT940 job : listJob){
					...
					receiverFTP.execute(job.getTypeTrx(), job.getIsSameAccountNo(), job.getFilePattern(), job.getTransactionCode(), job.getEndpointCode(), "bni_syariah");
					exportToXls.execute(job.getAccountNo(), job.getTypeTrx(), job.getTransactionCode(), job.getIsSameAccountNo(), "bni_syariah");
				}
	...
	public List<JobMT940> getJobs(){
		...
		try {
			...
			String sql = "SELECT * from job_mt940_bni_sya";
