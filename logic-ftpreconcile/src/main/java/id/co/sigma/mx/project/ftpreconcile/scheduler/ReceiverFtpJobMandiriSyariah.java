...
import id.co.sigma.mx.project.ftpreconcile.process.ExportToXlsMandiriSyariah;
import id.co.sigma.mx.project.ftpreconcile.process.ReceiverFTPMandiriSyariah;

public class ReceiverFtpJobMandiriSyariah extends QuartzJobBean{

	protected static transient Logger logger = Logger.getLogger(ReceiverFtpJobMandiriSyariah.class);

	private ReceiverFTPMandiriSyariah receiverFTP;
	private ExportToXlsMandiriSyariah exportToXls;
	private DataSource dataSource;
	private static boolean isRunning = false;

	public void setReceiverFTP(ReceiverFTPMandiriSyariah receiverFTP) {
		this.receiverFTP = receiverFTP;
	}
	
	public void setExportToXls(ExportToXlsMandiriSyariah exportToXls) {
		...
		while (isRunning) {
			try {
				...
				for(JobMT940 job : listJob){
					...
					receiverFTP.execute(job.getTypeTrx(), job.getIsSameAccountNo(), job.getFilePattern(), job.getTransactionCode(), job.getEndpointCode(), "mandiri_syariah");
					exportToXls.execute(job.getAccountNo(), job.getTypeTrx(), job.getTransactionCode(), job.getIsSameAccountNo(), "mandiri_syariah");
				}
	...
	public List<JobMT940> getJobs(){
		...
		try {
			...
			String sql = "SELECT * from job_mt940_bmri_sya";
