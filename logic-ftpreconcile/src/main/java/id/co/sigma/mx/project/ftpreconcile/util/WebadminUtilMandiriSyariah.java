...
public class WebadminUtilMandiriSyariah {
	...
	public boolean isEverInserted(String filename, String transactionType, String bankName){
		...
		try {
			...
			if (rs.next()) {
				...
				if (count > 0) {
					flag = true;
					logger.info("mandiri, Data already exist in table transaction_report webadmin, filename : " + filename);
				}
			}
		} catch (SQLException e) {
			logger.info("mandiri, Error in webadminUtil.isEverInserted function");
			logger.trace(e);
		}finally {
			if(s != null) {
				try {
	...
	public void insertData(Date transactionDate, String filename, String transactionType, String bankName){
		...
		try {
			...
			s.executeUpdate();
			logger.info("mandiri, Success insert into table transaction_report webadmin, filename : " + filename);
		} catch (SQLException e) {
			logger.info("mandiri, Error in webadminUtil.insertData function");
			logger.trace(e);
		}finally {
			if(s != null) {
				try {
