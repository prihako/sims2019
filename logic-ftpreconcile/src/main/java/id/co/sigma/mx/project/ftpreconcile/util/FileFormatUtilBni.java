 ...
 */
public class FileFormatUtilBni {
	...
	private static Logger logger = Logger.getLogger(FileFormatUtilBni.class);
		...
		try {
			...
			if (index > -1) {
				...
				if (curl != -1) {
					...
					logger.info("[FileFormatUtil.init] Error parsing file transaction : Cant parse format ${DATE:yyyyMMdd} from file pattern "
							+ FILE_PATTERN);
				}
			} else {
				logger.info("[FileFormatUtil.init] Error parsing file transaction : Cant parse format ${DATE:yyyyMMdd} from file pattern "
	...
	public static void init(String filePattern) {
		try {
			...
			logger.info("bni, init, FILE_PATTERN " + FILE_PATTERN);
		} catch (Exception e) {
			logger.error("bni, could not init ftp configuration, "
					+ e.getMessage());
		}
	}
	...
	public static String getFileNameDatePlusAccountNo(int count, String filePattern) {
		...
		String accountNumber = filePattern.substring(filePattern.lastIndexOf("_") + 1, filePattern.lastIndexOf("."));

		if (count == 0) {// Format the current time.
			Date currentTime = new Date();
			return formatter.format(currentTime) + "+"+ accountNumber;
		} else {
			Calendar cal = Calendar.getInstance();
			cal.add(Calendar.DAY_OF_YEAR, count);
			return formatter.format(cal.getTime()) + "+" + accountNumber;
		}
	...
	public static Calendar getTrasactionDate(String fileName) {
		...
		if (idx != -1) {
			...
			try {
				...
				logger.error(
						"bni, [FileFormatUtil.getTrasactionDate] Error parsing file transaction : ",
						e);
			}
		}
...
//	public static Calendar getTrasactionDate(String fileName, String filePattern) {
//
//		if (!isInit)
//			init(filePattern);
//
...
//			} catch (ParseException e) {
//				logger.error(
//						"[FileFormatUtil.getTrasactionDate] Error parsing file transaction : ",
//						e);
//			}
//		}
//
...
//	}
	...
	public static Calendar getTrasactionDate(String filenameDatePlusAccountNo, String datePattern, int count) {
		Calendar cal = Calendar.getInstance();
		String filenameDate = filenameDatePlusAccountNo.substring(0,filenameDatePlusAccountNo.indexOf("+"));
		try {
	...
	public static boolean isNewMonthTransaction(String fileName) {
		...
		try {
			...
			logger.error(
					"[FileFormatUtil.isNewMonthTransaction] Error parse : ", e);
		}
		...
		if (idx != -1) {
			...
			if (listDay.contains(dateFile.toLowerCase())) {
				...
				try {
					...
					logger.error(
							"[FileFormatUtil.isTransactionFile] Error file transaction : ",
							e);
					flag = false;
				}
			}
	...
	/**
	 * overloading from last method, to get day of transaction file from
	 * calendar
	 * 
	 * @param transactionDate
	 * @return day of transaction file
	...
	public static String getPrevFilename(String fileName, String filePattern) {
		Calendar calendar = getTrasactionDate(fileName, filePattern, 0);
	...
	public static String getPrevFilenameDate(String fileNameDatePlusAccountNo, String datePattern) {
		String rekeningNumber = fileNameDatePlusAccountNo.substring(fileNameDatePlusAccountNo.indexOf("+") + 1);
		...
		sb.append(sYear).append(sMonth).append(sDate).append("+").append(rekeningNumber);
