...
//    private static String FILE_PATTERN="1030061777773MT940${DATE:yyyyMMdd}";
    ...
    private static Logger logger = Logger.getLogger(FileFormatUtil.class);
    ...
    public static void init(String filePattern) {
		try {
	        ...
	        logger.info("mandiri, init, FILE_PATTERN " + FILE_PATTERN);
	        logger.info("mandiri, init, HEAD_FILE " + HEAD_FILE);
	        logger.info("mandiri, init, DATE_PATTERN " + DATE_PATTERN);
		} catch (Exception e) {
			logger.error("could not init ftp configuration, " + e.getMessage());
		}
    }
...
//        if (!isInit) init(type);
    ...
    public static Calendar getTrasactionDate(String fileName, int type) {
        if (!isInit) init(type);
...
//        String filename = "POSTEL-06-11-01.txt";
        String filename = "1030061777773MT94020060910";
//        String filename = "testingdata.txt";
        Calendar cal=Calendar.getInstance();
        String day=getDayFileTransaction(cal);
        System.out.println("day = " + day);
        String filenam=getFileName(0);
        System.out.println("filenam = " + filenam);
        String file=getFileName(0);
        System.out.println("file = " + file);
        file=getFileName(-1);
        System.out.println("file = " + file);
        if (isNewMonthTransaction(cal)) {
            System.out.println("awal bulan");
        } else {
            System.out.println("bukan awal bulan");
        }

        System.out.println(isTransactionFile(filename));

        Calendar c=getTrasactionDate(day);
