 ...
 */
public class MT940ParserMandiriSyariah {

    private static Logger logger = Logger.getLogger(MT940ParserMandiriSyariah.class);
    ...
    /**
     * used only to parse record transaction and put to list of bean (Transaction) for reor and skor
     *
     * @param recordTransac -> record2 transaction MI940
     * @param mt940
     * @return flag -> true if valid, else false
     ...
     * @throws SQLException
     */
    public boolean parseRecordToBeanSkorReor(List<String> recordTransac, MT940 mt940) throws SQLException, ClassNotFoundException {
    ...
    public boolean parseRecordToBean(List<String> recordTransac, MT940 mt940, String sameAccountNoFlag, 
    	...
    	logger.info("mandiri, Enter parseRecordToBean, sameAccountFlag : " + sameAccountNoFlag);
    	logger.info("mandiri, Enter parseRecordToBean, endpointCode : " + endpointCode);
        ...
        if (!recordTransac.isEmpty()) {
            ...
            for (Iterator<String> i = recordTransac.iterator(); i.hasNext() && isValidFile ;) {
                ...
                if(sameAccountNoFlag != null){
                	if( sameAccountNoFlag.equalsIgnoreCase("y") || sameAccountNoFlag.equalsIgnoreCase("yes") || sameAccountNoFlag.equalsIgnoreCase("1")){
                        String transactionCode = null;
                		int transactionCodeLength = transactionCodeParam.length();
                		String accountNo = FileFormatUtil.getFilePattern();
                		...
                		if(flagBit.equalsIgnoreCase("86")){
	                		if(secCode.length() > transactionCodeLength){
				                transactionCode = secCode.substring(secCode.length() - transactionCodeLength, secCode.length());
                			...
                			if(firstCode.length() > transactionCodeLength){
                				transactionCode = firstCode.substring(firstCode.length() - transactionCodeLength, firstCode.length());
                ...
                if(secCode.length() > 6){
	                ...
	                if(paymentChannel != null){
	                	...
	                	transaction.setPaymentChannel("mandiri, Unknown : " + channelId);
	                }
                }else{
                	transaction.setPaymentChannel("mandiri, unknown");
                }
                ...
                //This if use to check is there any error in transactionCode that have same account no,
...
//            }
            ...
            if (transacType==null) {
                transaction.setErrorDesc("Transaction type Tidak Ditemukan");
                return false;
            }
...
//    }
    ...
    private static boolean parseSecondRec(Transaction transaction, String strSecond){
    	if(strSecond.length() > 27) {
    		if(strSecond.startsWith("UBP")){
        		transaction.setInvoiceId(strSecond.substring(20,27));
        		transaction.setClientId(strSecond.substring(27,strSecond.length()));
        		transaction.setPaymentType(PostelConstant.HOST_TO_HOST);
        		return true;
        	}else {    		
//        		transaction.setErrorDesc("Error parsing code 86, format harus dimulai dengan UBP");
        		transaction.setPaymentType(PostelConstant.NOT_HOST_TO_HOST);
        		return true;
        	}
    	}else {
    		if(strSecond.startsWith("UBP")){
	    		transaction.setErrorDesc("Error parsing code 86, panjang string kurang dari 28 digit");
	    		return false;
    		}else{
    			transaction.setPaymentType(PostelConstant.NOT_HOST_TO_HOST);
        		return true;
    	...
    	if(strSecond.length() > 27) {
    		if(strSecond.startsWith("UBP")){
    			...
    			if(endpointCode.equals(PostelConstant.BILS0)){
    				...
    				Integer idx = strSecond.lastIndexOf(year);
    				Boolean isError = false;
    				
    				List<Integer> listIdxOccurences = getIdxOccurences(strSecond, year);
    				if(listIdxOccurences.isEmpty()){
    					year = String.valueOf(Integer.valueOf(year) - 1);
    					listIdxOccurences = getIdxOccurences(strSecond, year);
    				...
    				if(!isError){
    					invoiceId = strSecond.substring(20, idx + year.length());
    					clientId = strSecond.substring(20+invoiceId.length());
    				}else{
    					invoiceId = strSecond.substring(20,31);
    					clientId = strSecond.substring(31,strSecond.length());
    				}
    			}else{
    				invoiceId = strSecond.substring(20,30);
    				clientId = strSecond.substring(30,strSecond.length());
    			}
...
//        		transaction.setErrorDesc("Error parsing code 86, format harus dimulai dengan UBP");
    ...
    public List<Integer> getIdxOccurences(String text, String textToSearch){
		int lastIndex = 0;
	...
	//            if (recordFile.length > 2 && recordFile[0].indexOf("BANK MANDIRI") != -1 && recordFile[0].indexOf("PTABC") != -1) {
	            if(code62.length > 1) {
	            	if(code25.length > 1) {
		            	String filePattern = FileFormatUtil.getFilePattern();
		            	//cek rekening valid
    ...
    public String getFileTransacStatus(MT940 mt940, String fileName, String sameAccountNoFlag, 
    	...
    	logger.info("mandiri, getFileTransacStatus, sameAccountNoFlag : " + sameAccountNoFlag);
    	logger.info("mandiri, getFileTransacStatus, endpointCode : " + endpointCode);
		                    ...
		                    if (code20.length>1) {
								...
								if (transactionDate.equals(fileDate)) {
									...
									try {
		                        	 ...
		                        	 message = PostelConstant.STATUS_FILE_INVALID;
		                             logger.trace("mandiri, [MT940Parser.isValidFileTransac] transaction date between tag 20 and filname is different.");
		                        }
		                    } else {
		                    	message = PostelConstant.STATUS_DATE_NA;
		                        logger.trace("mandiri, [MT940Parser.isValidFileTransac] no found transaction date in code 20.");
		                    }
		            	}else {
		            		message = PostelConstant.STATUS_ACCOUNT_INVALID;
		            		logger.trace("mandiri, [MT940Parser.isValidFileTransac] invalid account number.");
		            	}
		            } else {
		            	message = PostelConstant.STATUS_ACCOUNT_NA;
		            	logger.trace("mandiri, [MT940Parser.isValidFileTransac] no found account number in code 25.");
		//                logger.error("[MT940Parser.isValidFileTransac] no found header MANDIRI and POSTEL in code 1 and 2.");
		            }
	            } else{
	            	message = PostelConstant.STATUS_INCOMPLETED_FILE;
	        	...
	        	message = PostelConstant.STATUS_CONTENT_NA;
	            logger.error("mandiri, [MT940Parser.isValidFileTransac] empty file.");
	        }
    	} catch (ParseException e1) {
            message = PostelConstant.STATUS_DATE_INVALID;
            logger.error("mandiri, [MT940Parser.isValidFileTransac] invalid transaction date in tag 20.");
            e1.printStackTrace();
        }
