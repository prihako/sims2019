...
public class ProcessorMessage {
	protected static transient Logger logger = Logger.getLogger(ProcessorMessage.class);
    ...
    public synchronized void processTransaction(FileTransactionReceive fileTransac,boolean isSpecialFolder, String sameAccountNoFlag, 
    	...
    	logger.info("mandiri, processTransaction, sameAccountNoFlag : " + sameAccountNoFlag);
    	logger.info("mandiri, processTransaction, bankName : " + bankName);
    	logger.info("mandiri, processTransaction, endpointCode : " + endpointCode);
        ...
        if (fileStatus.equals(PostelConstant.STATUS_VALID)) {
            ...
            if (!isProcessed) { //not processed yet
                ...
                if (!listTransaction.isEmpty()) {
                    ...
                    logger.info("File transaction (" + fileName + ") is parsed successfully.");
                    ...
                    try {
                        ...
                        logger.error("[ProcessorMessage.processTransaction] Error process while inserting valid file transaction, ", e);
                        throw e;
                    }

                } else { //list of model is empty,
                    logger.info("File transaction (" + fileName + ") is parsed successfully but no found transaction in file.");
                    ...
                    try {
                        ...
                        logger.error("[ProcessorMessage.processTransaction] Error process while inserting empty file transaction, ", e);
                        throw e;
                    }
                }
            } else {
                logger.info("This file ( "+  fileName  +" ) has been processed, will be received only.");

                try {
                	paymentUtil.saveFileTransactionLog(fileTransac, PostelConstant.STATUS_IGNORED, bankName);
                } catch (Exception e) {
                    logger.error("[ProcessorMessage.process] Error process while inserting ignored file transaction, ", e);
                    throw e;
                }
            }
        } else {
            logger.info("Invalid format in file transaction [ " + fileName + " ]");
            ...
            try {
                ...
                logger.error("[ProcessorMessage.processTransaction] Error process while inserting invalid file transaction, ", e);
                throw e;
            }
        }

        logger.info("PROSESS FILE TRANSACTION IS COMPLETE.");
