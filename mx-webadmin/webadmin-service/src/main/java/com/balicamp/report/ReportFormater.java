...
public class ReportFormater {
	 ...
	 */
	public ReportContent formatSparatedValue(List<String> datas, List<ColumnParam> colums, boolean isFixedLength)
		throws RawDataFormatException {
		...
		for (String data : datas) {
			...
			for (ColumnParam colum : colums) {
				...
				if (doubleCol) {
					...
					for (int i = 0; i < arrData.length; i++) {
						String raw = arrData[i];
						if (raw.indexOf("/") >= 0 && raw.indexOf("=") >= 0) {
							String splittedRaw[] = raw.split("/");
							String fieldName = splittedRaw[2].trim();
							if (fieldName.equals(colum.getFieldName())) {
				...
				if (null != value) {
					...
					if (null == value || value.trim().length() == 0) {
						value = (colum.getTipeData().startsWith(ColumnParam.RAW_TIPE_NUMERIC) ? "0" : " - ");
						strSumValue = value;
					} else {
						/*
						 * kondisi data harus di format sesuai konfigurasi di
						 * tabel m_mx_colum_param field fnq_formater
						 * setting map nya ada di definisi bean spring di file
						 * xml, terpaksa di hardcode karena parameter class
						 * formater yang terpisah-pisah dan menerima parameter
						 * yang beda-beda
						 * bila terjadi exception maka data yang ditampilkan
						 * tidak diformat
						 */
						try {
							if (null != colum.getKeyDataFormater()) {
								value = formatData(colum.getKeyDataFormater(), value, colum.getTipeData());
							}
						} catch (Exception e) {
							// todo nothing
						}
					...
					/*
					 * mensumary kan data yang harus disummarikan berdasarkan
					 * flag summary di column
					 */
					...
					/*
					 * menghitung row count untuk masing-masing conditional yang
					 * ada di konfigurasi
					 */
					if (colum.isRowCount()) {
			...
			if (!tmpBuffer.toString().trim().equals("")) {
				String tmp = ","
						+ (isFixedLength ? formatFixedLength(String.valueOf(rowCounter), lengthNumberCOlum,
								ColumnParam.RAW_MARGIN_CENTER) : String.valueOf(rowCounter)) + ","
						+ tmpBuffer.toString();
				bfr.append(tmp);
				rowCounter++;
			}
		...
		// convert summaryMap menjadi summary list<string> dimana menggabungkan
		// key dan value dan memformat angkanya menjadi 1.000
	...
	private String formatData(String dataFormater, String value, String tipeData) {
		...
		try {
			...
			if (null != formater) {
				...
				case 1: { // key MONEY_FORMAT result 1000000 --> 1.000.000
					// jika numeric baca dulu berapa decimal
					if (tipeData.startsWith("NUMERIC")) {
						String[] split = tipeData.split(",");
						if (split.length > 1) {
							int decimalCount = Integer.parseInt(split[1]);
							BigDecimal val = new BigDecimal(value.substring(0, value.length() - decimalCount) + "."
									+ value.substring(value.length() - decimalCount));
							value = MoneyFormat.getFormattedMoney(val);
						} else {
				...
				throw new RuntimeException("format data " + dataFormater
						+ " belum di support silahkan hubungi programer terdekat ");
			}
	 ...
	 * @param rowCountMap
	 */
	private void countRowCount(ColumnParam colum, String value, Map<String, Integer> rowCountMap) {
	 ...
	 * @throws RawDataFormatException
	 */
	private void countSummary(ColumnParam colum, String value, Map<String, Double> summaryMap)
		throws RawDataFormatException {
		if (colum.isSum()) {
	...
	private StringBuffer cereateHeaderTable(List<ColumnParam> colums, int lengthNumberCOlum, boolean isFixLength) {
		StringBuffer bfr = new StringBuffer();
		bfr.append(","
				+ (isFixLength ? formatFixedLength("NO", lengthNumberCOlum, ColumnParam.RAW_MARGIN_CENTER)
						: lengthNumberCOlum) + ",");
	...
	private StringBuffer loopHeader(StringBuffer bfr, List<ColumnParam> colums, boolean isKey) {
		...
		for (ColumnParam columnParam : colums) {
			String strColumn = formatFixedLength(isKey ? columnParam.getFieldName() : columnParam.getFieldString(),
					columnParam.getLength(), ColumnParam.RAW_MARGIN_CENTER);
	...
	public ReportHeader createHeader(ReportParamHeader headerParam) {
		...
		// Nama Bank
		String bank = formatFixedLength(headerParam.getBank(), headerParam.getReportLength(),
				ColumnParam.RAW_MARGIN_CENTER);
		header.setBank(bank);

		// judul
		String judul = formatFixedLength(headerParam.getJudul(), headerParam.getReportLength(),
				ColumnParam.RAW_MARGIN_CENTER);
		...
		// userId
		header.setUserId(formatFixedLength(headerParam.getUserId(), headerParam.getReportLength(),
				ColumnParam.RAW_MARGIN_LEFT));
		...
		header.setPeriode(formatFixedLength(bfr.toString(), headerParam.getReportLength(),
				ColumnParam.RAW_MARGIN_CENTER));
