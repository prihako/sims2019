...
public interface UserManager extends IManager, UserDetailsService {
	...
	User addNew(String fullName, Date dob, String cardNumber, String plainPassword, List<String> accountList);

	User addNew(User user, String plainPassword, List<String> accountList);

	User addNewAdmin(String userName, String name, String email, String plainPassword, Role role, long userParentId);
	...
	User reRegistration(User existingUser, String fullName, Date dob, String plainPassword, List<String> accountList);
	...
	List<UserDisplay> findUser(SearchCriteria searchCriteria, int firstResult, int maxResult);

	List<UserGroupDisplay> findAdminUser();

	List<UserGroupDisplay> findByGroupId(Long groupId);

	AuditLog linkNewCard(User user, String cardNumber, String newCardNumber, User actor);

	User resetUserAdminPassword(User actor, User user, String ipAddress);

	User resetUserToInquiryOnly(User user);

	AuditLog actionTransaction(User user, String type, Long TransactionId, String ReffNo);
	...
	AuditLog viewAccountBalanceDetail(User user, String accountNo, String ipAddress);
	...
	AuditLog changeUserSignUpStatus(User actor, UserRegister user, String ipAddress);
	...
	AuditLog adminEditParameter(User actor, SystemParameter systemParameter, String ipAddress);
}
