...
public class UserManagerImpl extends AbstractManager implements UserManager, InitializingBean {
	...
	public User addNewAdmin(String userName, String name, String email, String plainPassword, Role role,
		...
		try {
			userSingle.setEnabled(Boolean.TRUE);
			userSingle.setAccountExpired(Boolean.FALSE);
			userSingle.setAccountLocked(Boolean.FALSE);
			userSingle.setCredentialExpired(Boolean.FALSE);
			userSingle.setCredentialsExpired(false);
			userSingle.setCreatedBy(SecurityContextUtil.getCurrentUserId());
			userSingle.setCreatedDate(new Date());
			userSingle.setMustChangePassword(Boolean.TRUE);
			((AdminGenericDaoImpl) userDao).setIdentifier(userSingle);

			userDao.saveUserSingle(userSingle);

			// userDao.getCurrentHibernateTemplate().merge(userSingle);
	...
	@Override
	public AuditLog adminEditParameter(User actor, SystemParameter systemParameter, String ipAddress) {
		AuditLog auditLog = auditLogManager
				.save(ModelConstant.ReffNumType.CHANGE, ModelConstant.AuditLogSubType.PARAMETER_MANAGEMENT,
						"auditLog.info.format.change.editParameter", new Object[] { systemParameter
								.getSystemParameterId().getName(), ipAddress }, actor.getId(), new Date());
		return auditLog;
	}
	...
	public List<User> findAllUser() {
		...
		List<User> list = userDao.findAllEnabled();
		return list;
	}
}
