...
public abstract class ReconcileWeekly extends AdminBasePage implements PageBeginRenderListener, PageEndRenderListener {

	@InjectSpring("reconcileReportWeeklyManagerImpl")
	public abstract ReconcileReportWeeklyManager getReconcileReportWeeklyManager();
	...
	public ReconcileWeekly() {
		super();
		this.xlsFile = new XlstoStringConverter();
	}

	protected final Log log = LogFactory.getLog(ReconcileWeekly.class);
	...
	public abstract ReconcileReportWeekly getReconcileReportWeekly();
		...
		if (!pageEvent.getRequestCycle().isRewinding()) {
			if (!isNotFirstLoad()) {
				...
				if (getFields() == null) {
					...
					getFields().remove("RECONCILE_REPORT_WEEKLY");
				}
			}
		}
	}
	...
	public List getReconcileReportList() {
		List<ReconcileReportWeekly> list = (List<ReconcileReportWeekly>) getFields().get("RECONCILE_REPORT_WEEKLY");
	...
	public void doSearch(IRequestCycle cycle) {
		...
		List<ReconcileReportWeekly> list = getReconcileReportWeeklyManager().
				findReconcileReportWeeklyByDateAndReportType(getBankName(), getTransactionType(), getStartDate(), getEndDate(), reportType);

		// for add serial number, so it can display in table html
		int i = 0;
		for (ReconcileReportWeekly trx : list) {
			++i;
			trx.setSerialNumber(i);
		}

		if (list == null || list.size() < 1) {
			addError(getDelegate(), "errorShadow", getText("leftmenu.reconcileWeekly.errorMessage.emptySearchResult"), ValidationConstraint.CONSISTENCY);
		}

		getFields().put("RECONCILE_REPORT_WEEKLY", list);
	...
	public String validation() {
		...
		if (getStartDate() == null) {
			errorMessage = getText("leftMenu.reconcileWeekly.startDate.null");
		} else if (getEndDate() == null) {
			errorMessage = getText("leftMenu.reconcileWeekly.endDate.null");
		} else if (getEndDate().before(getStartDate())) {
			errorMessage = getText("leftMenu.reconcileWeekly.startDate.bigger.endDate");
		}
		...
		if (errorMessage == null) {
			...
			if (getEndDate().after(cal.getTime())) {
				errorMessage = getText("leftMenu.reconcileWeekly.startDate.max3Month");
			}
		}
	...
	public void retrieveFilePdf(IRequestCycle cycle, ReconcileReportWeekly reconcileReport, String type) {

		String reportType = type.equalsIgnoreCase("Unsettled/Need Confirmation") ? "unsettled" : type.toLowerCase();

		String reportName = reconcileReport.getReportDocument() + "_" + reportType + "_" + reconcileReport.getBankName() + ".pdf";
		...
		System.out.println("DOWNLOAD --> " + context + "/download?fileName=" + reportName + "&transactionType=" + reconcileReport.getTransactionType() + "&bankName=" + reconcileReport.getBankName());

		cycle.sendRedirect(context + "/download?fileName=" + reportName + "&transactionType=" + reconcileReport.getTransactionType() + "&reconcileTypeReport=weekly"+ "&bankName=" + reconcileReport.getBankName());
	}

	public void retrieveFileXls(IRequestCycle cycle, ReconcileReportWeekly reconcileReport, String type) {

		String reportType = type.equalsIgnoreCase("Unsettled/Need Confirmation") ? "unsettled" : type.toLowerCase();

		String reportName = reconcileReport.getReportDocument() + "_" + reportType + "_" + reconcileReport.getBankName() + ".xls";
	...
	public String cekFileInFileSystem(String remoteFile, String transactionType, String bankName) {
		...
		String reportPath = (getSystemParameterManager().findParamValueByParamName("reconcileWeekly.report.path"));
		...
		File file = new File(reportPath + "/" + remoteFile.substring(17, 38) + "/" + remoteFile);

		if (!file.exists()) {
			errorMessage = getText("leftMenu.reconcileWeekly.ftp.fileNotFound");
			return errorMessage;
		}
