...
import com.balicamp.webapp.action.common.WaitingPage;

/**
 * @version $Id: BasePage.java 517 2013-06-25 10:13:25Z rudi.sadria $
 */
public abstract class BasePage extends org.apache.tapestry.html.BasePage {
	...
	/*
	 * public User getUserLoginFromSession() { return new User("admin1",
	 * "balicamp123", true, false, false, false, null); }
	 */
	...
	/*
	 * public Format initDateFormat() { return new
	 * SimpleDateFormat(getText(ValidationConstants.DATE_PATTERN_KEY)); }
	 */
	...
	public void cekDoubleSubmit() {
		// state tidak valid
		if (getDoubleSubmitKey() == null) {
			getRequestCycle().sendRedirect("doubleSubmit.jsp");
			// FIXME: throw new
			// ApplicationException("pemanggilan cekDoubleSubmit tetapi doubleSubmitKey null");
		} else {
			Boolean cekBoolean = (Boolean) getSessionPropertyValue(getDoubleSubmitKey());

			if (cekBoolean == null) {
				getRequestCycle().sendRedirect("doubleSubmit.jsp");
				// FIXME: throw new DoubleSubmitException("double submit");
			}
	...
	/*
	 * protected void checkValidateToken() { TokenAuth tokenAuth = (TokenAuth)
	 * getComponent("tokenAuth"); MethodResponse<?> methodResponse =
	 * tokenAuth.isAuthSessionValid(); if (!methodResponse.isStatus()) {
	 * addError(getDelegate(), tokenAuth.getInputComponent(),
	 * getText(methodResponse.getMessageKey()),
	...
	protected void download(String file) {
		...
		try {
			...
			System.out.println("DOWNLAOD WORD DOKUMEN --> " + context + "/download?fileName=" + file);
	...
	public void onCancel(IRequestCycle cycle) {
		cycle.sendRedirect(getPrevPage());
	}

	@Persist("session")
	public abstract HashMap getModel();

	public abstract void setModel(HashMap model);

	protected HashMap getData() {
		HashMap data = new HashMap();
		String transactionDate = new SimpleDateFormat("MMddHHmmss").format(new Date());

		String stan = "" + getSystemParameterManager().getSequenceNextValue("stan_seq");

		data.put("assocKey", stan);
		data.put("transactionDateTime", transactionDate);
		data.put("mti", "RQ");
		data.put("userId", getUserLoginFromSession().getUsername());
