...
import com.balicamp.service.impl.jms.sender.DeliveryChannelRequestSender;
...
public abstract class WaitingPage extends BasePage implements PageBeginRenderListener, PageEndRenderListener {
	private static final Log log = LogFactory.getLog(WaitingPage.class);

	@InjectObject("spring:mrm")
	public abstract MRM getMrm();

	@InjectObject("spring:deliveryChannelRequestSender")
	public abstract DeliveryChannelRequestSender getSender();
	...
	public void pageBeginRender(PageEvent pageEvent) {
		setCheckInterval(getSystemParameterManager().getLongValue(
				new SystemParameterId("other", "other.waitPage.refreshInterval"), 5));
		// setCheckInterval(3);

		IRequestCycle cycle = pageEvent.getRequestCycle();

		ILink link = checkResponse(cycle);
		if (link != null) {
	...
	public ILink send(WaitingPageCommand command, HashMap request, IRequestCycle cycle) {
		if (command == null) {
		...
		command.setDeliveryChannelId((String) request.get("DeliveryChannelId"));
		return getEngineService().getLink(false, getPageName());
	}

	protected ILink checkResponse(IRequestCycle cycle) {
		...
		if (mrm.isTimeOut(dcId)) {
			return getTimeoutLink(cycle);
		}

		HashMap response = mrm.getResponse(dcId);
		if (response == null) {
			return null;
		}
	...
	protected ILink getNextLink(IRequestCycle cycle) {
		...
		if (CommonUtil.isNotEmpty(nextPageInitMethod)) {
			try {
				MethodUtils.invokeMethod(cycle.getPage(nextPage), nextPageInitMethod, command.getResponse());
			} catch (Exception e) {
				...
				log.error("an error occured while invoking next page init method: " + nextPageInitMethod + ".", e);
			}
		}
		...
		if (paramList != null) {
			try {
				MethodUtils.invokeMethod(cycle.getPage(nextPage), "setParamList", paramList);
			} catch (Exception e) {
				if (e instanceof InvocationTargetException) {
					InvocationTargetException e2 = (InvocationTargetException) e;
					Throwable e3 = e2.getTargetException();
	...
	protected ILink getErrorLink(IRequestCycle cycle) {
		...
		String errorPageInitMethod = command == null ? null : command.getErrorPageInitMethod();

		if (CommonUtil.isNotEmpty(errorPage)) {
			if (CommonUtil.isNotEmpty(errorPageInitMethod)) {
				try {
					MethodUtils.invokeMethod(cycle.getPage(errorPage), errorPageInitMethod, command);
				} catch (Exception e) {
					log.error("an error occured while invoking error page init method: " + errorPageInitMethod + ".", e);
				}
			}
		...
		HashMap response = command == null ? null : command.getResponse();
		if (response == null) {
			infoPageCommand.addMessage(getText("waitSendJms.failResponseCode.message.0"));
		} else {
			infoPageCommand.addMessage(getText("waitSendJms.failResponseCode.message", getText("response.code."
					+ command.getResponse().get("responseCode"))));
		}
	...
	protected ILink getTimeoutLink(IRequestCycle cycle) {
		...
		if (CommonUtil.isNotEmpty(timeoutPage)) {
			if (CommonUtil.isNotEmpty(timeoutPageInitMethod)) {
				try {
					MethodUtils.invokeMethod(cycle.getPage(timeoutPage), timeoutPageInitMethod, command);
				} catch (Exception e) {
					log.error("an error occured while invoking timeout page init method: " + timeoutPageInitMethod
							+ ".", e);
				}
			}
