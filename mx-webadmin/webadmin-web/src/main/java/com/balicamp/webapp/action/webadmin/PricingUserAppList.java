 ...
 * 
 */
public abstract class PricingUserAppList extends BasePageList implements
		PageBeginRenderListener {
	private static final Logger LOGGER = Logger
			.getLogger(PricingUserAppList.class.getSimpleName());
		...
		return new IBasicTableModel() {
			...
			public int getRowCount() {
				try {
					if (getChannelCodeItem() != null) {
						return getApprovalManager().findCountPricingUserApproval(getUserLoginFromSession().getId(),
								getIdentifier());
					} else {
						return getApprovalManager().findCountPricingUserApproval(getUserLoginFromSession().getId());
					}
			...
			public Iterator<TransactionFee> getCurrentPageRows(int nFirst,
				...
				try {
					...
					if (getChannelCodeItem() != null) {
						apprs = getApprovalManager().findPricingUserApproval(getUserLoginFromSession().getId(),
								getIdentifier(), nFirst, nPageSize);
					} else {
						apprs = getApprovalManager().findPricingUserApproval(getUserLoginFromSession().getId(), nFirst, nPageSize);
					}
