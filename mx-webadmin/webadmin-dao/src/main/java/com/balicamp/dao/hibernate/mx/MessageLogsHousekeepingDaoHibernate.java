...
@Repository
public class MessageLogsHousekeepingDaoHibernate extends MxGenericDaoHibernate<MessageLogs, Long> implements MessageLogsHousekeepingDao {
	...
	@Override
	public List<?> getMessageLogsByProperty(MessageLogsParameter param, int firsResult, int maxResult) {
		StringBuffer buffer = new StringBuffer();
		setQueryArgument(buffer, param);
		Query query = getSession()
				.createQuery(
						"SELECT m.id.transactionId, m.id.conversionTime, m.id.endpointCode, m.id.mappingCode, m.id.isRequest,"
								+ "tlog.id.channelRc, tlog.id.transactionTime, tlog.id.transactionCode, "
								+ "mp.description, "
								+ "tr.name,"
								+ "e.id, "
								+ "m.id.raw "
								...
								+ "AND tlog.id.transactionCode=tr.code " + buffer.toString()
	...
	@Override
	public List<?> getCountMessageLogsByProperty(MessageLogsParameter param) {
		StringBuffer buffer = new StringBuffer();
		setQueryArgument(buffer, param);
		Query query = getSession()
				.createQuery(
