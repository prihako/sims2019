...
public class TransactionFee extends BaseAdminModel implements java.io.Serializable {
	...
	private String key; //because no primary key
	...
	public String getKey() {
		this.key = getId().getTransactionId()+"-"+getId().getChannelId()+"-"+getId().getIdentifier();
		return key;
	}
	...
	@AttributeOverrides({ @AttributeOverride(name = "transactionId", column = @Column(name = "transaction_id",
			...
			length = 100,nullable = true)), @AttributeOverride(name = "fee", column = @Column(name = "fee", nullable = false)) })
