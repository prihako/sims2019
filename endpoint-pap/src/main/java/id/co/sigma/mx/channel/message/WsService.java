...
public class WsService implements Transport<WsMessage>,InitializingBean,DisposableBean {
	...
	public void write(WsMessage message) {
		...
		try{
			...
			if (LOGGER.isTraceEnabled()) {
				LOGGER.trace("###Receiving message from MX At PAP Endpoint###");
			}
			
			if(message.getValue("transactionType").equalsIgnoreCase("PAP.INQ")){
				...
				LOGGER.trace("+++Sent message inquiry to PAP+++");
				...
				LOGGER.trace("+++Receive response from  PAP : " + response.toString());
			...
			}else if(message.getValue("transactionType").equalsIgnoreCase("PAP.PAY")){
				...
				LOGGER.trace("+++Sent message payment to PAP+++");
				...
				LOGGER.trace("+++Receive response payment from  PAP : " + response);
	
				message.setValue("errorCode", response.getStatus().getErrorCode());
				message.setValue("responseCode", response.getStatus().getErrorCode());
			}else if(message.getValue("transactionType").equalsIgnoreCase("PAP.REV")){
				StringArray stringArray = new StringArray();
				ArrayOfString arrayOfString = new ArrayOfString();
				stringArray.getItem().add(message.getValue("paidBills"));
				arrayOfString.setStrings(stringArray);
				...
				LOGGER.trace("+++Sent request Reversal to  PAP+++");
				...
				LOGGER.trace("+++Receive response reversal from  PAP : " + response);
			...
			LOGGER.error("error something", e);
		}
	}
	...
	private void tellListeners(WsMessage response) {
		for (MessageListener<WsMessage> listener : listeners) {
			if(LOGGER.isTraceEnabled())
				LOGGER.trace("Notify MX for response message from PAP");
	...
	public void destroy() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Shutting Down PAP Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "disconnected");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("PAP Endpoint successfully stopped");
	}
...
//	};
	...
	public void afterPropertiesSet() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Starting Up PAP Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "ready");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("PAP Endpoint successfully started");
