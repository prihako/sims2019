...
import id.co.sigma.mx.channel.perangkat.ArrayOfString;
import id.co.sigma.mx.channel.perangkat.InquiryRequest;
import id.co.sigma.mx.channel.perangkat.InquiryResult;
import id.co.sigma.mx.channel.perangkat.PaymentManagerControllerPortType;
import id.co.sigma.mx.channel.perangkat.PaymentRequest;
import id.co.sigma.mx.channel.perangkat.PaymentResult;
import id.co.sigma.mx.channel.perangkat.ReversalRequest;
import id.co.sigma.mx.channel.perangkat.ReversalResult;
import id.co.sigma.mx.channel.perangkat.StringArray;
...
public class WsService implements Transport<WsMessage>,InitializingBean,DisposableBean {
	...
	public void write(WsMessage message) {
		...
		if (LOGGER.isTraceEnabled()) {
			LOGGER.trace("###Receiving message from MX At PERANGKAT Endpoint###");
		}
		
		if(message.getValue("transactionType").equalsIgnoreCase("PER.INQ")){
			...
			LOGGER.trace("+++Sent message inquiry to PERANGKAT+++");
			...
			LOGGER.trace("+++Receive response from  PERANGKAT : " + response.toString());
		...
		}else if(message.getValue("transactionType").equalsIgnoreCase("PER.PAY")){
			...
			LOGGER.trace("+++Sent message payment to PERANGKAT+++");
			...
			LOGGER.trace("+++Receive response payment from  PERANGKAT : " + response);

			message.setValue("errorCode", response.getStatus().getErrorCode());
			message.setValue("responseCode", response.getStatus().getErrorCode());
		}else if(message.getValue("transactionType").equalsIgnoreCase("PER.REV")){
			StringArray stringArray = new StringArray();
			ArrayOfString arrayOfString = new ArrayOfString();
			stringArray.getItem().add(message.getValue("paidBills"));
			arrayOfString.setStrings(stringArray);
			...
			LOGGER.trace("+++Sent request Reversal to  PERANGKAT+++");
			...
			LOGGER.trace("+++Receive response reversal from  PERANGKAT : " + response);
	...
	private void tellListeners(WsMessage response) {
		for (MessageListener<WsMessage> listener : listeners) {
			if(LOGGER.isTraceEnabled())
				LOGGER.trace("Notify MX for response message from PERANGKAT");
	...
	public void destroy() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Shutting Down PERANGKAT Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "disconnected");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("PERANGKAT Endpoint successfully stopped");
	}
...
//	};
	...
	public void afterPropertiesSet() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Starting Up PERANGKAT Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "ready");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("PERANGKAT Endpoint successfully started");
