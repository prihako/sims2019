 ...
 * @version $Id: MessageLogsDaoHibernate.java 503 2013-05-24 08:13:16Z rudi.sadria $
 */
@Repository
public class MessageLogsDaoHibernate extends MxGenericDaoHibernate<MessageLogs, Long> implements MessageLogsDao {
	...
	protected void setQueryArgument(StringBuffer buffer, MessageLogsParameter param) {
		...
		if (param.getStartDate() != null) {
			buffer.append(" AND m.id.conversionTime >= :startDate");
		}
		if(param.getEndDate() != null) {
			buffer.append(" AND m.id.conversionTime <= :endDate");
		}
		if(param.getRawKey() != null) {
			if(param.getRawKey().length()>0)
