...
public abstract class UploadSuratLainnya extends AdminBasePage implements PageBeginRenderListener {
...
//		String fileName = file.getFileName();
		...
		fileName = fileName.replace("/","_"); 
		fileName = fileName + "_" + getTimestampStr() + ".pdf";
...
//	To set file after page reload
	...
	public void doBack(IRequestCycle cycle){
		...
		upDoc.setActiveSKL(true);
		
		upDoc.setJenisSKL(criteria);
		upDoc.setKriteriaSKL(searchCriteria);
...
//	For check, if document exist, prompt user for replacement
	...
	public String generalValidation(){
		...
		if(getFileName() == null){
			errorMessage = getText("leftMenu.masterMaintenance.ipsfr.uploadDokumen.fileName.empty");
		}else if(getDeskripsi() == null){
	...
	public void updateData(){
		List<UploadView> listData = getUploadViewManager().findDataByClientName("IN-6", uploadView.getClientName());
