...
public  class RawStringColumnParam extends ColumnParam{
	...
	private RawStringColumnParam(String fieldName, String fieldString, String tipeData,int length, String margin, boolean  isSum,boolean isRowCount, Map<String, String> condition) {		
	...
	private boolean validateFormat(String[] colum) throws RawColumnFormatException{
		...
		if(null == colum || colum.length != 7) throw new RawColumnFormatException(RAW_FORMAT);
		
		
		if(colum[2].equals(ColumnParam.RAW_TIPE_NUMERIC) || colum[2].equals(ColumnParam.RAW_TIPE_DATE) || colum[2].equals(ColumnParam.RAW_TIPE_TEXT)){
			//todo nothing
		}else{
		...
		}catch (Exception e) {throw new RawColumnFormatException(RAW_FORMAT); }
		
				
		if(colum[4].equals(ColumnParam.RAW_MARGIN_LEFT) || colum[4].equals(ColumnParam.RAW_MARGIN_CENTER)|| colum[4].equals(ColumnParam.RAW_MARGIN_RIGHT))
		{ /*todo nothing*/  }
		else{throw new RawColumnFormatException(RAW_FORMAT);}
		

		if(colum[5].equals(ColumnParam.RAW_SUMMARY_YES) || colum[5].equals(ColumnParam.RAW_SUMMARY_NO)) 		{ /*todo nothing*/  }
		else{throw new RawColumnFormatException(RAW_FORMAT);}
		
		if(colum[6].equals(ColumnParam.RAW_COUNT_NO) || colum[6].indexOf(ColumnParam.RAW_COUNT_YES) > -1 ) 		{ /*todo nothing */ }
		else{throw new RawColumnFormatException(RAW_FORMAT);}
	...
	public List<ColumnParam> format() throws RawColumnFormatException {
		...
		if(null == colums || colums.length <=0 ) throw new RawColumnFormatException(RAW_FORMAT);		
		...
		for (int i = 0; i < colums.length; i++) {
			...
			if(isRowCount){
				// ROW_COUNT_Y{SUKSES;00,GAGAL;01} --> MAP( SUKSES (KEY) , 00 (VALUE))
			...
			RawStringColumnParam tmp = new RawStringColumnParam(colum[0],colum[1],colum[2],Integer.valueOf(colum[3]),colum[4],sum,isRowCount,condition);
