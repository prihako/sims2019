 ...
 *
 * @author Matt Raible
 * @version $Revision: 114 $ $Date: 2012-12-12 11:18:29 +0700 (Wed, 12 Dec 2012) $
 */
		...
		try {
			...
			if (data.getAttributeString("var") == null) {
				...
				for (int i = 0; i < fields.length; i++) {
					...
					vars.add(new VariableInfo(fields[i].getName(), ((fields[i].getType().isArray()) ? type.substring(2,
							type.length() - 1) + "[]" : type), true, VariableInfo.AT_END));
				}
			} else {
				String var = data.getAttributeString("var");
				String type = c.getField(var).getType().getName();
				vars.add(new VariableInfo(c.getField(var).getName(), ((c.getField(var).getType().isArray()) ? type
						.substring(2, type.length() - 1) + "[]" : type), true, VariableInfo.AT_END));
			}
		} catch (Exception cnf) {
