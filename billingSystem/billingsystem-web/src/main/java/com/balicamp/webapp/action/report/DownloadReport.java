...
public abstract class DownloadReport extends BasePage {
	...
	public abstract String getTglAwal();

	public abstract void setTglAwal(String tglAwal);

	public abstract String getTglAkhir();

	public abstract void setTglAkhir(String tglAkhir);
	...
	public void download(RequestCycle cycle) {
		...
		for (ReportModel tmpReport : getReports()) {
			...
			Date tglAkhir = null;
			try {
				tglAwal = DateUtil.convertStringToDate("MM/dd/yyyy", getTglAwal());
				tglAkhir = DateUtil.convertStringToDate("MM/dd/yyyy", getTglAkhir());
				/*Date limitDate = DateUtil.addDate(getTglAwal(), 4);
				if(tglAkhir.after(limitDate)) {
					setValidDate("FALSE");
					return;
				}*/
			} catch (Exception e) {
			...
			ReportParam rpt = new ReportParam(tmpReport.getId(), tglAwal, tglAkhir, isTxt(), isCsv());
