...
public abstract class UploadPencairanBG extends AdminBasePage implements PageBeginRenderListener {
	...
	public abstract void setLicenseNoH(String no);
	
	public abstract String getLicenseNoH();
	
	public abstract void setDocType(String type);
	
	public abstract String getDocType();
	
	public abstract void setRefNumber(String ref);
	
	public abstract String getRefNumber();
...
//			save file temporary before move to destination folder when user agree to replace document
		...
		getUserManager().uploadDokumen(initUserLoginFromDatabase(), getRequest().getRemoteHost(), "Pencairan Bank Garansi");
...
//	To set file after page reload
	...
	public void doBack(IRequestCycle cycle){
		...
		upDoc.setActivePBG(true);
		
		upDoc.setJenisPBG(criteria);
		upDoc.setKriteriaPBG(searchCriteria);
...
//	For check, if document exist, prompt user for replacement
	...
	public void updateData(){
		List<UploadView> listData = getUploadViewManager().findDataByClientName("BG-4", uploadView.getClientName());
