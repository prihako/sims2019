...
//	For Table
	...
	public abstract void setRemarksEdit(String remarksEdit);
...
//		}
		...
		if(invoice.getInvCreatedDate() != null){
			   setInvCreatedDateFineStr(dateFormat.format(invoice.getInvCreatedDate()));
			  }
...
//	@EventListener(targets = "saveNilaiBHPButton", events = "onclick", submitForm = "form-license")
...
/*		if (getNilaiBhpEdit()!=null){
					System.out.println("Invoice Id = "+getInvoice().getInvoiceId()+" ;Nilai BHP = "+getNilaiBhpEdit()+" ;Invoice Type = "+getInvoiceType());
					System.out.println("Saving Nilai Bhp Frekuensi");
					if(getInvoiceManager().updateNilaiBhpFrekuensi(Long.parseLong(getNilaiBhpEdit()), getInvoice().getInvoiceId(), getRemarks())>0){
		...
		getInvoice().setBhpTotalOri(new BigDecimal(Long.parseLong(getNilaiBhpEdit())));
		getInvoice().setInvoiceComment(getRemarksEdit());
