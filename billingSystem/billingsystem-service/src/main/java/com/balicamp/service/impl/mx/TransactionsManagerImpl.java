...
import com.balicamp.dao.mx.TransactionsHousekeepingDao;
...
public class TransactionsManagerImpl extends AbstractManager implements TransactionsManager {
	...
	@Autowired
	private TransactionsHousekeepingDao transactionsHousekeepingDao;
	...
	public List<TransactionsDto> findTransactions(Object txCode, Object txName, String channelCode, Date startDate,
		...
		List<Object> result = (List<Object>) transactionsDao.findTransactions(txCode, txName, channelCode, startDate,
				endDate, first, max);
		List<Object> resultHousekeeping = (List<Object>) transactionsHousekeepingDao.findTransactions(txCode, txName,
				channelCode, startDate, endDate, first, max);
		String rawResponseCodeSuspect = parameterManager.findByParamName("message.responseCode.suspect")
				.getParamValue();

		result.addAll(resultHousekeeping);
		...
		for (Object o : result) {
			...
			if (row.length > 4) {
				dto.setChannelCode((String) row[0]);
				dto.setTransactionName((String) row[1]);
				dto.setTransactionCode((String) row[2]);
				dto.setChannelRc((String) row[3]);
				dto.setCount(new BigDecimal(row[4].toString()).intValue());
			} else {
				dto.setChannelCode((String) row[0]);
				dto.setTransactionCode((String) row[1]);
				dto.setTransactionName(transactionsDao.findByFieldName(dto.getTransactionCode()));
				dto.setChannelRc((String) row[2]);
				dto.setCount(new BigDecimal(row[3].toString()).intValue());
			}

			if (dto.getChannelRc() == null) {
				dto.setChannelRc("ZZ");
			}

			String key = getLogKeys(new String[]{dto.getChannelCode(), dto.getTransactionName(), dto.getTransactionCode()});
			TransactionsDto tmpDto = logsMap.get(key);
			if (tmpDto == null) {
				dto.setGagal(0);
				dto.setTimeout(0);
				dto.setSukses(0);
				setChannelRc(dto.getChannelRc(), dto.getCount(), dto, responseCode);
				logsMap.put(key, dto);
			} else {
				setChannelRc(dto.getChannelRc(), dto.getCount(), tmpDto, responseCode);
			}
		}
	...
	public int getRowCount(Object txCode, Object txName, String channelCode, Date startDate, Date endDate) {
		return (transactionsHousekeepingDao.getRowCount(txCode, txName, channelCode, startDate, endDate) + transactionsDao
				.getRowCount(txCode, txName, channelCode, startDate, endDate));
	}
