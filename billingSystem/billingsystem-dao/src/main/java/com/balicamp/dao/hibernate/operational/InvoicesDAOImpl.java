...
public class InvoicesDAOImpl extends AdminGenericDaoImpl<Invoice, Long>
	...
	@Override
	public int updateNilaiBhpFrekuensi(Long nilaiBhpFrekuensi, Long invoiceId, String remarks) {
		Invoice invoice = findInvoiceByID(invoiceId);
		if (invoice.getBhpTotalOri() == null || invoice.getBhpTotalOri() == new BigDecimal(0)){
			String sqlExtra = "update ebs.t_invoice t set t.bhp_total_ori=:totalBhp where t.invoice_id=:invoiceId";
			Query queryExtra = getSessionFactory().getCurrentSession().createSQLQuery(sqlExtra);
			queryExtra.setParameter("totalBhp", invoice.getBhpTotal());
			queryExtra.setParameter("invoiceId", invoiceId);
			if(queryExtra.executeUpdate()>0){
				System.out.println("BHP TOTAL ORIGINAL UPDATED");
			}
		}
	
		String sql = "update ebs.t_invoice t set t.invoice_comment_bhp=:remarks, t.bhp_total=:totalBhp where t.invoice_id=:invoiceId";
		Query query = getSessionFactory().getCurrentSession().createSQLQuery(sql);
		query.setParameter("totalBhp", nilaiBhpFrekuensi);
		query.setParameter("invoiceId", invoiceId);
		query.setParameter("remarks", remarks);
		int updatedInvoice = query.executeUpdate();
