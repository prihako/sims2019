...
public class WsService implements Transport<WsMessage>,InitializingBean,DisposableBean {
	...
	public void write(WsMessage message) {
		...
		if (LOGGER.isTraceEnabled()) {
			LOGGER.trace("###Receiving message from MX At UNAR Endpoint###");
		}
		
		if(message.getValue("transactionType").equalsIgnoreCase("UNAR.INQ")){
			...
			LOGGER.trace("+++Sent message inquiry to UNAR+++");
			...
			LOGGER.trace("+++Receive response from  UNAR : " + response.toString());
		...
		}else if(message.getValue("transactionType").equalsIgnoreCase("UNAR.PAY")){
			...
			LOGGER.trace("+++Sent message payment to UNAR+++");
			...
			LOGGER.trace("+++Receive response payment from  UNAR : " + response);

			message.setValue("errorCode", response.getStatus().getErrorCode());
			message.setValue("responseCode", response.getStatus().getErrorCode());
		}else if(message.getValue("transactionType").equalsIgnoreCase("UNAR.REV")){
			StringArray stringArray = new StringArray();
			ArrayOfString arrayOfString = new ArrayOfString();
			stringArray.getItem().add(message.getValue("paidBills"));
			arrayOfString.setStrings(stringArray);
			...
			LOGGER.trace("+++Sent request Reversal to  UNAR+++");
			...
			LOGGER.trace("+++Receive response reversal from  UNAR : " + response);
	...
	private void tellListeners(WsMessage response) {
		for (MessageListener<WsMessage> listener : listeners) {
			if(LOGGER.isTraceEnabled())
				LOGGER.trace("Notify MX for response message from UNAR");
	...
	public void destroy() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Shutting Down UNAR Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "disconnected");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("UNAR Endpoint successfully stopped");
	}
...
//	};
	...
	public void afterPropertiesSet() throws Exception {
		if(LOGGER.isInfoEnabled())
			LOGGER.info("Starting Up UNAR Endpoint");
		this.endpointStatusManager.updateStatus(endpointCode, "ready");
		if(LOGGER.isInfoEnabled())
			LOGGER.info("UNAR Endpoint successfully started");
